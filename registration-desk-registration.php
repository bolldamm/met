...
$resultInits = $db->callProcedure("CALL ed_sp_obtener_listado_conferencia_inits('$numeroConferencia','1')");
...
//  $codeProcedure = "CALL ed_sp_obtener_listado_conferencia_completo('$numeroConferencia','2'";
  $codeProcedure = "CALL ed_sp_obtener_listado_conferencia_completo('$numeroConferencia','1'";
} else {
//  $codeProcedure = "CALL ed_sp_obtener_listado_conferencia_filtrado('$numeroConferencia','".$attendeeName."','2'";
  $codeProcedure = "CALL ed_sp_obtener_listado_conferencia_filtrado('$numeroConferencia','".$attendeeName."','1'";
}
...
if ($db->getNumberRows($resultMiembros) > 0) {
    while ($dataMiembro = $db->getData($resultMiembros)) {
      ...
      if($dataMiembro["id_usuario_web"]){
        $badgelines[1] = "MET member ";
      } elseif($dataMiembro["id_asociacion_hermana"]) {
        $badgelines[1] = "Sister member ";
      } else {
        $badgelines[1] = "Non-member ";
      }
      ...
      $badgelines[1] = $badgelines[1] . $firstTimer . $speakerHelper;
      
      $id_attendee = $dataMiembro["id_inscripcion_conferencia"];
      
      $result = $db->callProcedure("CALL ed_pr_get_guests($id_attendee,7)");
      $row = $result->fetch_assoc();
      $total_reception_guests = $row["valor"]; 
      
      $result = $db->callProcedure("CALL ed_pr_get_guests($id_attendee,2)");
      $row = $result->fetch_assoc();
      $total_dinner_guests = $row["valor"];
      
      $badgelines[2] = "";
      
      if ($total_reception_guests || $total_dinner_guests) {      
      $badgelines[2] = "Reception guests = " . $total_reception_guests . " / Dinner guests = " . $total_dinner_guests;     
      }
      
      if ($dataMiembro["observaciones"]) {
        if ($badgelines[2]) {
          $badgelines[2] = $badgelines[2]."<br />".$dataMiembro["observaciones"];
        } else {
          $badgelines[2] = $dataMiembro["observaciones"];
        }
      } 
      
      if ($dataMiembro["comentarios"]) {
        if ($badgelines[2]) {
          $badgelines[2] = $badgelines[2]."<br /><em>".$dataMiembro["comentarios"]."</em>";
        } else {
          $badgelines[2] = "<em>".$dataMiembro["comentarios"]."</em>";
        }
      } 

        $subPlantilla->assign("ITEM_MIEMBRO_ONCLICK", "location.href='"."/register.php?attendee=".$dataMiembro["id_inscripcion_conferencia"]."&name=".str_replace("'","%27",$badgelines[0])."'");
       	$subPlantilla->assign("ITEM_MIEMBRO_BADGE1", "<a href='"."/register.php?attendee=".$dataMiembro["id_inscripcion_conferencia"]."&name=".str_replace("'","%27",$badgelines[0])."'>".$badgelines[0]."</a>");
