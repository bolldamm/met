<?php

/**
 * 
 * Pagina de inicio desde donde se inicia el portal
 * @author Edata S.L.
 * @copyright Edata S.L.
 * @version 1.0
 */
require "includes/load_main_components.inc.php";

if($_SESSION["met_user"]["tipoUsuario"] < TIPO_USUARIO_EDITOR)
{
	if(!isset($_SESSION["registration_desk"]))
		{
			generalUtils::redirigir(CURRENT_DOMAIN);
		}
}

$attendeeNumber = $_POST['attendeeId'];
$attendeeNotes = generalUtils::escaparCadena($_POST['txtContenido']);
$db->callProcedure("CALL ed_pr_set_conference_registered(" . $attendeeNumber . ",'". $attendeeNotes ."',true)");

generalUtils::redirigir(CURRENT_DOMAIN . "/en/registration:1408");

?>