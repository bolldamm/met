...
$cutoffHour = 13;        // Hour (24h format) – include everything before this

// ---- Weekday ordering for comparison ----
$dayOrder = [
    "Monday"    => 1,
    "Tuesday"   => 2,
    "Wednesday" => 3,
...
$pagefooter .= 'The workshops have an interactive component.<br>';
...
$content = '<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Pre-Conference Workshops</title>';
...
foreach ($lines as $lineIndex => $line) {
    ...
    // Skip lines where title is exactly "Desk open"
...
$content .= '<div style="display: flex; align-items: center; margin-bottom: 20px;">
        ...
        <h1 style="margin: 0;">PRE-CONFERENCE WORKSHOPS</h1>
      </div>';
...
foreach ($allSchedules as $day => $data) {
    ...
    if ($day === $cutoffDay) {
        // Only include sessions before cutoff hour
        $filtered = [];
        ...
        // Any day before cutoff day is included in full
...
function renderTable($day, $rooms, $schedule) {
    ...
    foreach ($schedule as $slot) {
        ...
        if ($talkCount === 0) {
            ...
            if ($slot['extra']) {
                $txt .= ' <small>(' . htmlspecialchars($slot['extra']) . ')</small>';
            }
            if (!empty($slot['room'])) {
                $txt .= '&nbsp;[' . htmlspecialchars($slot['room']) . ']';
            }
            ...
            if (!empty($talk['room'])) {
                $out .= ' [' . htmlspecialchars($talk['room']) . ']';
            }
            $out .= '</td>';
        } else {
            foreach ($rooms as $r) {
                ...
                if ($talk) {
                    $out .= '<td>'
                        . htmlspecialchars($talk['title']) . '<br/>'
                        . '<em>' . htmlspecialchars($talk['presenter']) . '</em>'
                        . '</td>';
                } else {
                    $out .= '<td></td>';
                }
