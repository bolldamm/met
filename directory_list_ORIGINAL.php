...
require "includes/load_main_components.inc.php";
	...
	$wholeWord = 0;
        ...
        $targetLanguage="";
        $areaExpertise="";
	//$filtro="#c_#t_#p_#w_#ci_#so_#ta_#ae";
		...
		if($db->getNumberRows($resultMiembros) > 0) {
			while($dataMiembro = $db->getData($resultMiembros)) {
				...
				if($dataMiembro["correo_electronico"] != "") {
					...
					$subPlantilla->parse("contenido_principal.listado_miembros.tr.item_miembro.mail");
				}
				...
				if($dataMiembro["web"] != "") {
					...
					$subPlantilla->parse("contenido_principal.listado_miembros.tr.item_miembro.web");
				}
    ...
    $subPlantilla->assign("COMBO_AREAS_OF_EXPERTISE", generalUtils::construirCombo($db, "CALL ed_sp_get_areas_of_expertise(".$_SESSION["id_idioma"].")", "ae", "cmbAreas", $areaExpertise, "nombre", "id_area", STATIC_DIRECTORY_MEMBER_DATABASE_SEARCH_AREAS_OF_EXPERTISE, 0, "class='inputText left' style='width:272px;'"));

	//Source languages dropdown   
    $subPlantilla->assign("COMBO_SOURCE_LANGUAGES", generalUtils::construirCombo($db, "CALL ed_sp_get_source_languages(".$idMenuTipo.",".$_SESSION["id_idioma"].")", "so", "cmbSrc", $sourceLanguage, "nombre", "id_working_language", STATIC_DIRECTORY_MEMBER_DATABASE_SEARCH_SOURCE_LANGUAGE, 0, "class='inputText left' style='width:272px;'"));

	//Target languages dropdown   
    $subPlantilla->assign("COMBO_TARGET_LANGUAGES", generalUtils::construirCombo($db, "CALL ed_sp_get_target_languages(".$idMenuTipo.",".$_SESSION["id_idioma"].")", "ta", "cmbTgt", $targetLanguage, "nombre", "id_working_language", STATIC_DIRECTORY_MEMBER_DATABASE_SEARCH_TARGET_LANGUAGE, 0, "class='inputText left' style='width:272px;'"));

	//Countries dropdown
	$subPlantilla->assign("COMBO_PAISES", generalUtils::construirCombo($db, "CALL ed_sp_web_usuario_web_pais_obtener_combo(".$idMenuTipo.")", "c", "cmbPais", $idPais, "nombre_original", "id_pais", STATIC_DIRECTORY_MEMBER_DATABASE_SEARCH_COUNTRY, 0, "class='inputText left' style='width:272px;'"));
