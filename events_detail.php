<?php
	...
	$subPlantilla = new XTemplate("html/event_detail.html");
	...
	if(isset($_GET["event"]) && is_numeric($_GET["event"])) {
		$resultNoticia = $db->callProcedure("CALL ed_sp_web_agenda_obtener(".$_SESSION["id_idioma"].", ".$_GET["event"].")");
		if($db->getNumberRows($resultNoticia) > 0) {
			...
			$vectorAtributosDetalle["id_detalle"] = $dataNoticia["id_agenda"];
