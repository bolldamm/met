...
function refrescarCaptcha(){
	document.getElementById("image").src = "../classes/secureimage/securimage_show.php?sid=" + Math.random();
...
function expandirSeccion(contenedorIcono,contenidoSeccion){
	var contenedor=document.getElementById(contenedorIcono);
	var contenidoSeccion=document.getElementById(contenidoSeccion);
	if(contenedor.className=="iconExpand"){
		contenedor.className="iconCollapse";
		contenidoSeccion.style.display="none";
	}else{
		contenedor.className="iconExpand";
		contenidoSeccion.style.display="";
	}
}
...
function obtenerComboModalidadUsuario(invocador){
	var contenedor=document.getElementById("comboModalidad");
	var contenedorLabel=document.getElementById("labelComboModalidad");
	...
	if(invocador.value==0){
		...
		ajax.onreadystatechange=function(){
			if(ajax.readyState==4){
				...
				//Si somos nominee, escondemos ciertos valores que no aporta nada al ser nominee
				if(invocador.value==4){
					$(".noNominee").css("display","none");
					//Mostramos el combo de institucion asociada
					$(".institucionAsociada").css("display","none");
					$(".institucionAsociada").css("display","");
				}
			}
...
function gestionarBloqueNominee(invocador){
	if(invocador.value==4){
		$(".noNominee").css("display","none");
		$(".institucionAsociada").removeClass("noDisplayed");
		$(".institucionAsociada").addClass("displayed");
	}else{
		$(".noNominee").css("display","");
		$(".institucionAsociada").removeClass("displayed");
		$(".institucionAsociada").addClass("noDisplayed");
	}
}
