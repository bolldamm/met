<?php
	...
	if(count($_POST)){
		...
		/*$resultado=$db->callProcedure("CALL ".OBJECT_DB_ACRONYM."_sp_movimiento_editar(".$idMovimiento.",".$_POST["cmbTipo"].",".$_POST["cmbSubConcepto"].",".$_POST["cmbTipoPago"].",'".generalUtils::escaparCadena($_POST["txtConceptoPersonalizado"])."','".generalUtils::escaparCadena($_POST["txtPersona"])."','".generalUtils::conversionFechaFormato($_POST["txtFecha"])."','".$descripcion."','".$_POST["txtImporte"]."',".$pagado.",'".$Comment."')");*/	


		
		//Si el concepto es membership fee, es decir new member or renew membership, and the movement is marked paid...

		
		/******** INICIO: email para membership ********/
		...
		if($_POST["cmbConcepto"]==34 && $pagado==1){
			...
			/******** FIN: mail para membership ********/
		}else if($_POST["cmbSubConcepto"]==55 && $pagado==1){
					
			/******** INICIO: mail para workshop-register ********/
			...
			if($db->getNumberRows($resultadoMovimientoInscripcion)>0){
				...
				if($datoMovimientoInscripcion["es_mail_enviado"]==0){
					...
					/*
					$nombre="";
					if($datoMovimientoInscripcion["tratamiento"]!=""){
						$nombre.=$datoMovimientoInscripcion["tratamiento"]." ";
					}
					$nombre.=$datoMovimientoInscripcion["nombre"]." ".$datoMovimientoInscripcion["apellidos"];
                    */
			...
			/******** FIN: mail para workshop-register ********/
			
			/******** INICIO: mail para conference-register ********/
			...
			if($db->getNumberRows($resultadoMovimientoInscripcion)>0){
				...
				//If email not sent yet and payment type not Paypal, send email and set registration as paid and email as sent
				if($datoMovimientoInscripcion["id_tipo_pago_movimiento"]!=10 && $datoMovimientoInscripcion["es_mail_enviado"]==0){
					...
					$subPlantillaMail->assign("MAIL_INSCRIPTION_TO_USER_NAME_VALUE",$nombre);
                    */
					$firstName=$datoMovimientoInscripcion["nombre"];		
					$subPlantillaMail->assign("MAIL_INSCRIPTION_TO_USER_NAME_VALUE",$firstName);
		...
		/******** FIN: mail para conference-register ********/
