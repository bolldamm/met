<?php
	...
	if(count($_POST)){
		...
		if($_FILES["fileExcel"]["tmp_name"]!=""){	
			require "classes/excel/excel_reader.php";
			
			$data = new Spreadsheet_Excel_Reader($_FILES["fileExcel"]["tmp_name"]);
			$totalFilas=$data->rowcount();
			for($i=2;$i<=$totalFilas;$i++){
				$destinatarios[$totalDestinatarios]=$data->val($i, 1);
				$totalDestinatarios++;
				//array_push($destinatarios,$data->val($i, 1));
			}
		}
		...
		if($totalMiembrosSeleccionados>0){
			...
			for($i=0;$i<$totalMiembrosSeleccionados;$i++){
				...
				//$destinatarios[$totalDestinatarios]["id_miembro"]=$miembros[$i];
				$totalDestinatarios++;
			}
		}
