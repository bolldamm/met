<?php
	...
	if(count($_POST)){
		...
		$resultado=$db->callProcedure("CALL ".OBJECT_DB_ACRONYM."_sp_taller_editar(".$idTaller.",'".$enlace."',".$miniSesion.",".$activo.")");
	...
	while($datoIdioma=$db->getData($resultadoIdioma)){
		...
		if($i==0){
			...
			$subPlantilla->assign("TALLER_FECHA_INICIO",generalUtils::conversionFechaFormato($datoTaller["fecha_inicio"]));
			$subPlantilla->assign("TALLER_FECHA_FIN",generalUtils::conversionFechaFormato($datoTaller["fecha_fin"]));
