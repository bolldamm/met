<?php
	...
	if(count($_POST) > 0) {
		...
		if($txtTitulo != "" && $txtTitulo != STATIC_FORM_MEMBERSHIP_TITLE  && $txtResumen != "" && $txtResumen != STATIC_FORM_EVENT_SUBMIT_SUMMARY && $txtComentario != "" && $txtComentario != STATIC_FORM_EVENT_SUBMIT_CONTENT) {
			...
			if(isset($_SESSION["met_user"])){
			$mailSubPlantilla->parse("logged-in_member");
			}else{
			$mailSubPlantilla->parse("non-logged-in_member");
			}		
			...
			$mail->AddAddress('jobs@metmeetings.org');
		...
		//				$idUsuarioWebCorreo=$_SESSION["met_user"]["id"];
				...
				array_push($vectorDestinatario,'jobs@metmeetings.org');
