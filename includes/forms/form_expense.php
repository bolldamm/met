<?php
	...
	if(count($_POST) > 0) {
		...
		$resultMovimiento = $db->callProcedure("CALL ed_sp_web_movimiento_insertar(".$tipoMovimiento.",".$concepto.",1,".$tipoPago.",".$_SESSION["met_user"]["id"].",'".$nombre."','".generalUtils::conversionFechaFormato($fecha)."','".$contenido."','".$importe."',".$esPagado.")");
	...
	$plantillaFormulario->assign("COMBO_TIPO_PAGO",generalUtils::construirCombo($db, "CALL ed_sp_web_tipo_pago_movimiento_obtener_combo(".$_SESSION["id_idioma"].")", "cmbTipoPago", "cmbTipoPago", -1, "nombre", "id_tipo_pago_movimiento", STATIC_EXPENSE_FORM_DESIRED_PAYMENT, "", "class='form-control selected required' required"));
